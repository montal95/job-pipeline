...
**Date:** 2026-03-24
**Branch:** main
**Test state:** 64/64 passing (Phase 0–2 + config CLI complete)
...
src/pipeline/
  cli.py          ✅ config set/edit/show commands added (committed 8337bd8)
scripts/
  save_auth.py    ✅ Multi-tab Chrome login helper
tests/
  test_config_cli.py  ✅ 13 tests for _set_env_key and _read_env_keys
docs/
...
1. **Resume format:** Use the table-based layout from the master `.docx`. `_render_resume_docx` should reproduce it structurally, not approximate it.

2. **Output directory:** Confirmed `settings.output_dir` (`./output/`). Already in `.gitignore`. Naming: `{job_id}_resume.docx` / `{job_id}_cover_letter.docx`.

3. **`pipeline config` CLI:** ✅ Already implemented and committed (`8337bd8`). `pipeline config set ANTHROPIC_API_KEY <key>` writes to `.env`; `pipeline config edit` opens `.env` in the system editor; `pipeline config show` prints masked values. 13 tests passing. 64/64 total.

---

## Open questions for session start

None outstanding — all three standing questions resolved above. Begin directly with Commit 1 (`pypdf` dependency).
